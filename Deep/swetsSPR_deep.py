...
item_file = 'swets_deep_items.txt'
...
if not os.path.exists(os.getcwd() + '/swets_deep_results/'): os.makedirs(os.getcwd() + '/swets_deep_results/')
...
#### Create output
data = open(os.getcwd() + "/swets_deep_results/" + str("%03d" % (current_subject_number,)) + ".txt", "w")      ## Open output file channel for editing
...
def main():
    ...
    for trialNum, item in enumerate(experimental_items):
        ...
        for word in response:
            data = open(os.getcwd() + "/swets_deep_results/" + str("%03d" % (current_subject_number,)) + ".txt", "a")
